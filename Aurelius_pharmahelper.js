...
function checkLowStock() {
    const threshold = 10;
    const lowStockMeds = medications.filter(function (med) {
...
function createPrescriptionReport() {
    console.log("\nPrescription Report:");
...
function addPrescription(patientName, medName, quantity) {
...
function addMedicine(name, stock, price) {
    const existingMed = medications.find(m => m.name.toLowerCase() == name.toLowerCase());
    if (existingMed) {
        console.log(`Medication '${name}' already exists.`);
        return;
    }

    const maxId = medications.reduce(function (max, m) {
        return m.id > max ? m.id : max;
    }, 0);

    const newId = maxId + 1;
    medications.push({ id: newId, name: name, stock: stock, price: parseFloat(parseFloat(price).toFixed(2)) });
...
function calculateTotalMedicationsSold() {
    let totalSold = 0;
    let totalRevenue = 0;
...
function restockMedication(medName, quantity) {
    const med = medications.find(m => m.name.toLowerCase() == medName.toLowerCase());
...
// If the medication does not exist, output an error.
function searchMedication(medName) {
    const med = medications.find(m => m.name.toLowerCase().includes(medName.toLowerCase()));
...
// If the medication does not exist, output an error.
function updateMedication(id, newName, newPrice) {
...
// export the node module at the end of the file.
module.exports = {
    medications,
    prescriptions,
    checkLowStock,
